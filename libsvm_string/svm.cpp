...
private:
...
#ifdef _STRING
	static double edit(const char *px, const char *py);
	...
	{
		return exp(-gamma * edit(x[i].s,x[j].s));
	}
#else
	double kernel_linear(int i, int j) const
	{
...
#define OLD
	...
	row[0] = (double*) malloc( (len1+1) * sizeof(double) );
	row[1] = (double*) malloc( (len1+1) * sizeof(double) );
	...
	//result=row[old][len1];
    //result = 1.0f - (similarity / double(std::min(len1, len2)));
    result = row[old][len1] / double(std::max(len1, len2));
   
	free(row[0]); free(row[1]);
   
	return result;
#else
    int len1 = (int)strlen(px);
    int len2 = (int)strlen(py);
    int similarity = 0;

    // init a table with dimensions len1+1 x len2+1, filled with 0
    std::vector<std::vector<double>> table(len1+1, std::vector<double>(len2+1, 0.0f));

    for (int i = 0; i < len1 + 1; i++)
        table[i][0] = i / (double)len1;

    for (int j = 0; j < len2 + 1; j++)
        table[0][j] = j / (double)len2;
    ...
    for (int i = 1; i < len1 + 1; i++)
    {
        ...
        {
            min = std::min(table[i - 1][j] + 1 / double(len1), table[i - 1][j] + 1 / double(len2));
            if (px[i - 1] != py[j - 1])
            {
                min = std::min(min, table[i - 1][j - 1] + 0.5f * (1 / double(len1) + 1 / double(len2)));
            }
            else
            {
                min = std::min(min, table[i - 1][j - 1]);
                similarity++;
            }
    ...
    double result = 1.0f - (similarity / double(std::min(len1, len2)));
    return result;
#endif //OLD
}
#endif
...
static void solve_one_class(
	...
	double *alpha, Solver::SolutionInfo* si)
{
    unsigned int l = prob->l;
	double *zeros = new double[l];
	schar *ones = new schar[l];
    unsigned int i;

    unsigned int n = (int)(param->nu*prob->l);	// # of alpha's at upper bound
	...
	double Cp, double Cn, double& probA, double& probB)
{
	...
	for(i=0;i<nr_fold;i++)
	{
        unsigned int begin = i*prob->l/nr_fold;
        unsigned int end = (i+1)*prob->l/nr_fold;
        unsigned int j,k;
